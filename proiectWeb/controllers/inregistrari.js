...
const getAllParticipantiMaterie = async (req,res) => {
	try{
		const inregistrari = await Inregistrare.findAll({where:{materiiId:req.body.materiiId}});
		if(inregistrari)
			res.status(200).json(inregistrari);
		else
			res.status(404).send({ message: "Inregistrarile nu au fost gasite" });
...
const getAllMateriiParticipant = async (req,res) => {
	try{
		const inregistrari = await Inregistrare.findAll({where:{utilizatoriId:req.body.utilizatoriId}});
		if(inregistrari)
			res.status(200).json(inregistrari);
		else
			res.status(404).send({ message: "Inregistrarile nu au fost gasite" });
